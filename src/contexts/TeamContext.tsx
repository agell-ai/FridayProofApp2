...
export interface TeamContextValue {
  ...
  createTeamMember: (memberData: TeamMemberInput) => TeamMember;
  updateTeamMember: (id: string, updates: Partial<TeamMemberInput>) => void;
...
export const TeamProvider = ({ children }: TeamProviderProps) => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (user.accountType === 'business') {
        filteredMembers = mockTeamMembers.filter(member =>
          ...
          member.projectIds.some(projectId => ['6', '7'].includes(projectId))
        );
      }
  ...
  const getTeamMembersByIds = useCallback(
    (ids: string[]) => teamMembers.filter(member => ids.includes(member.id)),
    [teamMembers]
  );
  ...
  const updateTeamMember = useCallback((id: string, updates: Partial<TeamMemberInput>) => {
    setTeamMembers(prev =>
      prev.map(member => {
        if (member.id !== id) {
          return member;
