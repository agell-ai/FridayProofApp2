...
export type MetricTone = keyof typeof TONE_CLASSES;


export type MetricDetailStat = {
  label: string;
  value: string;
  tone?: MetricTone;
};
...
export type MetricResult = MetricComputedValue & {
  id: string;
  label: string;
  description: string;
  tags?: string[];
...
export const formatHours = (value: number): string => `${formatNumber(value)} hrs`;
...
export type AutomationAnalyticsSummary = {
  tools: Tool[];
  totalTools: number;
  activeTools: Tool[];
  testingTools: Tool[];
...
export const metricCatalog: Record<MetricCategory, MetricDefinition[]> = {
  clients: [
    {
      id: 'active-clients',
...
export const computeClientAnalytics = (clients: Client[]): ClientAnalyticsSummary => {
...
export const computeProjectAnalytics = (projects: Project[]): ProjectAnalyticsSummary => {
...
export const computeTeamAnalytics = (teamMembers: TeamMember[]): TeamAnalyticsSummary => {
...
export const computeAutomationAnalytics = (tools: Tool[]): AutomationAnalyticsSummary => {
...
export const computeAllMetrics = (
  analyticsContext: MetricComputeContext,
): Record<MetricCategory, Record<string, MetricResult>> => {
...
export const computeMetricsForCategory = (
  analyticsContext: MetricComputeContext,
  category: MetricCategory,
): Record<string, MetricResult> => {
  return metricCatalog[category].reduce<Record<string, MetricResult>>((acc, definition) => {
    acc[definition.id] = {
      id: definition.id,
      label: definition.label,
      description: definition.description,
...
export const useMetricAnalytics = () => {
  const { clients } = useClients();
  const { projects } = useProjects();
  const { teamMembers } = useTeam();
  const { tools } = useTools();

  const clientAnalytics = useMemo(() => computeClientAnalytics(clients), [clients]);
  const projectAnalytics = useMemo(() => computeProjectAnalytics(projects), [projects]);
  const teamAnalytics = useMemo(() => computeTeamAnalytics(teamMembers), [teamMembers]);
  const automationAnalytics = useMemo(() => computeAutomationAnalytics(tools), [tools]);
  ...
  const computedMetrics = useMemo(() => computeAllMetrics(analyticsContext), [analyticsContext]);
