...
type MetricKey = 'total' | 'active' | 'prospects' | 'new';
...
type MetricBuckets = Record<MetricKey, Client[]>;
...
const Clients: React.FC = () => {
  ...
  const selectedRangeOption = useMemo(() => getTimeRangeOption(timeRange), [timeRange]);
  ...
  const { metricCounts, clientsByMetric } = useMemo(() => {
    ...
    const valueMatchesSearch = (value: string | null | undefined) =>
    ...
    clients.forEach((client) => {
      if (
        ...
        !(
          [
            ...
            client.website ?? '',
            client.linkedinUrl ?? '',
          ].some((value) => valueMatchesSearch(value)) ||
      ...
      const isUpdatedWithinRange = isWithinTimeRange(client.updatedAt, rangeStart);
      const isCreatedWithinRange = isWithinTimeRange(client.createdAt, rangeStart);

      if (isUpdatedWithinRange) {
        buckets.total.push(client);
        counts.total += 1;
        ...
        if (client.status === 'prospect') {
          buckets.prospects.push(client);
          counts.prospects += 1;
        }
      }

      if (isCreatedWithinRange) {
        buckets.new.push(client);
        counts.new += 1;
      }
    });

    return { metricCounts: counts, clientsByMetric: buckets };
  ...
  const hasFilters =
    normalizedSearch.length > 0 || selectedMetric !== 'total' || timeRange !== DEFAULT_TIME_RANGE;
  ...
  if (isLoading) {
    return (
      <div className="flex h-64 items-center justify-center">
        <div className="h-12 w-12 animate-spin rounded-full border-b-2 border-sunset-purple" />
      </div>
    );
  }
